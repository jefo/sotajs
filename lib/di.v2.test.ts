...
import { createPort, setPortAdapter, setPortAdapterWithDependencies, usePort, resetDI } from "./di.v2";
...
test("usePort should throw if no adapter is set", () => {
    ...
    expect(() => {
        usePort(myPort);
    }).toThrow("No implementation found for the port. Did you forget to call setPortAdapter() or setPortAdapterWithDependencies()?");
});
...
test("should handle dependencies between services", () => {
    ...
    setPortAdapterWithDependencies(userServicePort, userServiceFactory);
