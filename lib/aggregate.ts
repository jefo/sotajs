	...
	return class Aggregate {
		...
		public static create(initialData: unknown): Aggregate {
			...
			for (const invariant of config.invariants) {
				try {
					invariant(validatedProps);
				} catch (error) {
					// If an invariant fails during creation, re-throw the error
					console.error(
						`Invariant failed during ${config.name} creation:`,
						error,
					);
					throw error;
				}
		 ...
		 */
		public readonly actions = Object.keys(config.actions).reduce(
			(acc, actionName) => {
				acc[actionName as keyof TActions] = (...args: any[]) => {
					...
					const { state: nextState, event } = actionFn(this.props, ...args);
					...
					for (const invariant of config.invariants) {
						try {
							...
							console.error(
								`Invariant failed for ${config.name}.${actionName}:`,
								error,
							);
							throw error;
						}
			...
			{} as {
				[K in keyof TActions]: (...args: Parameters<TActions[K]>[1][]) => void;
			},
		);
	};
}
