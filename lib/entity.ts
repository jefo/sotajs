...
export type PropertySetters<TProps extends { id: string }> = {
	[K in Exclude<keyof TProps, "id"> as `set${Capitalize<string & K>}`]: (
		value: TProps[K],
	) => void;
};
 ...
 */
function generateSetters<TProps extends { id: string }>(
	schema: z.ZodType<TProps>,
): Record<string, (props: TProps, value: any) => void> {
	...
	if (schema instanceof z.ZodObject) {
		...
		Object.entries(shape).forEach(([key, validator]) => {
			if (key !== "id") {
				// Don't create setter for id as it shouldn't change
...
>(config: EntityConfig<TProps, TActions, TComputed>) {
	...
	const allActions = { ...config.actions, ...autoSetters } as Record<
		string,
		any
	>;
	...
	class Entity {
		 ...
		 */
		public get actions(): {
			[K in keyof typeof allActions]: (
				...args: (typeof allActions)[K] extends (
					props: TProps,
					...args: infer P
				) => any
					? P
