...
export class PrimaryBotAdapter {
  ...
  private async handleCallbackQuery(query: CallbackQuery) {
    ...
    try {
      ...
      if (data === "admin_tpl_menu") {
        ...
        const keyboard: InlineKeyboard = [
          [{ text: "💳 Оплата подтверждена", callback_data: "edit_tpl_payment_confirmed" }],
          [{ text: "🚀 Успешный онбординг", callback_data: "edit_tpl_join_success" }],
          [{ text: "❌ Отказ в доступе", callback_data: "edit_tpl_join_declined" }],
      ...
      if (data.startsWith("reset_tpl_")) {
        ...
        const session = getSession(query.from.id);
        session.step = "idle";
        await this.sendMessage(chatId, `✅ Этап <code>${key}</code> сброшен до стандарта.`, {
          parseMode: "HTML",
        });
        return this.answerCallbackQuery(query.id);
      }
      ...
      if (data === "admin_list_subscribers") {
        ...
        await this.sendMessage(chatId, `👥 <b>Резиденты акселератора (${subs.length}):</b>`, {
          parseMode: "HTML",
        });
      ...
      if (data.startsWith("admin_revoke_")) {
        ...
        await this.sendMessage(chatId, `✅ Резидент ${userId} успешно исключен из клуба.`);
        return this.answerCallbackQuery(query.id);
      }
  ...
  private async handleMessage(message: Message) {
    ...
    if (userId === this.adminId && session.step === "awaiting_template_content") {
    ...
    if (userId === this.adminId && session.step !== "idle") {
      switch (session.step) {
        case "awaiting_plan_name":
          ...
          await this.sendMessage(chatId, "Шаг 2/4: Укажите размер инвестиции в RUB:");
          break;
        case "awaiting_plan_price":
          ...
          await this.sendMessage(chatId, "Шаг 3/4: Укажите период участия (в днях):");
          break;
        case "awaiting_plan_duration":
          ...
          await this.sendMessage(chatId, "Шаг 4/4: Введите ID закрытого сообщества:");
          break;
    ...
    if (text === "/start") {
      ...
      if (args.length > 1 && args[1].startsWith("pay_")) {
        const planId = args[1].replace("pay_", "");
