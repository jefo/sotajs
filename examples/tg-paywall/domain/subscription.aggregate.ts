  ...
  actions: {
    activate: (state, durationDays: number) => {
      if (state.status !== "pending") {
        throw new Error(`Cannot activate subscription from state: ${state.status}`);
      }
      
      const now = new Date();
      state.status = "active";
      state.expiresAt = new Date(now.getTime() + durationDays * 24 * 60 * 60 * 1000);
      state.updatedAt = now;
      
      return {
        event: new SubscriptionActivatedEvent(state.id, state.userId, state.expiresAt),
      };
    },
    
    expire: (state) => {
      if (state.status !== "active") {
        throw new Error(`Cannot expire subscription from state: ${state.status}`);
      }
